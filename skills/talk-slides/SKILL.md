...
- Stamps, four per row, in a fenced code block so the columns line up. Each stamp is two lines: `N <On screen>` and the Visual as written (`empty`, or `number|photo|diagram|quote — <what it is>`). If Visual is unset, write `empty`. Shorten either line so the stamp fits; do not wrap onto a third line. No Seconds. No letter codes.
