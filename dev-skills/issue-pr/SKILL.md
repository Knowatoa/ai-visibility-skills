...
Confirm it lists every `skills/*/SKILL.md` and nothing from `dev-skills/`. Today that is `podcast-prep` and `talk-adversary`. `name` must match the folder. If you changed a skill (public or repo-only), also check frontmatter (`name`, `description` ≤ 1024 chars).
